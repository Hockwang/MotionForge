...
updated: 2026-04-23
status: implemented  # mvp3 合入 main（见 §11 决策 #9：扩到 17 段）
---
# 叉车取放模板（Forklift Pickup Template）

> **设计目的**：消除 AI 生成 PKF 时的数值误差和视觉瞬移。前端持有"行业标准取放 17 段"模板（14 必选 + 3 段可选横移），AI 只负责节奏和风格，数值由前端按几何推导并注入。
>
> **关键约束（来自中台）**：**输出 ZIP 格式一律不变**。模板是前端编译中间层，编译结果仍是标准 PKF（parameters + steps + reparent_events），中台内部编辑器照常读取。中台感知不到"模板"的存在。
>
> **状态**：已实现并上 main（mvp3 milestone）。场景无横移 role 时 optional 段 1/9/17 编译期跳过，自动降级为 14 段。
...
**职责拆分**：把"几何精度"从 AI 手里拿走，前端用行业标准 17 段模板（14 必选 + 3 段可选横移）+ 几何公式保证精度；AI 只管"节奏和风格"。
...
**所有 17 段默认全部保留（场景无横移 role 时 optional 段 1/9/17 编译期自动跳过，实际生成 14 段），由用户在生成后的 PKF 编辑器里手动删除不需要的段。**
...
- ❌ **段取舍判断**：17 段一律生成（无横移 role 时 optional 段自动跳过），用户决定是否手动删除（见 §4.4）
...
```
你是叉车动作节奏编排。前端已经按行业标准 17 段模板（14 必选 + 3 段可选横移）生成了完整的运动结构。
你只需要：
1. 为全部 17 段每段决定时长（即使场景无横移，段 1/9/17 也照给，前端会跳过）
2. 每段选一个 easing
3. 给整个 clip 起个名字
...
未来可扩展 `mode: 'pickup_only' | 'dropoff_only' | 'full'`。当前 MVP 只支持 full（17 段完整流程，无横移降级 14 段）。
...
🚀 一键生成触发
  ...
  │
  ├─ 四项全满足 → 模板路径
  │    └─ 按 §4 编译 17 段（无横移 role 时自动降级 14 段）→ 标准 PKF
  │
  └─ 任一缺失 → 老 L1/L2 AI 管道
                （自由生成，行为不变）
```
...
| 场景已布置 cargo+dropoff，但想改局部动作 | 🚀 生成完整 17 段（无横移则 14 段），**在 PKF 编辑器里手动删/改 step** |
...
1. 新文件 `src/core/ForkliftTemplate.js`
   - `FORKLIFT_TEMPLATE` 数据常量（17 段结构，14 必选 + 3 段可选横移）
